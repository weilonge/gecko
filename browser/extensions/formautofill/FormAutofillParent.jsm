...
function FormAutofillParent() {
  ...
  XPCOMUtils.defineLazyGetter(this, "profileStorage", () => {
    ...
    profileStorage.initialize().then(function onStorageInitialized() {
      ...
      log.debug("[TRACE] " + "updateSavedFieldNames done.");
    }.bind(this));

    log.debug("[TRACE] " + "return profileStorage.");
...
FormAutofillParent.prototype = {
  ...
  async init() {
    ...
    Services.ppmm.addMessageListener("FormAutofill:InitParent", this);
  ...
  receiveMessage({name, data, target}) {
    switch (name) {
      case "FormAutofill:InitParent": {
        log.debug("A: " + "this.profileStorage.initialize()");
      ...
      case "FormAutofill:SaveAddress": {
        if (data.guid) {
          log.debug("B: " + "this.profileStorage.addresses.update(data.guid, data.address)");
          this.profileStorage.addresses.update(data.guid, data.address);
        } else {
          log.debug("C: " + "this.profileStorage.addresses.add(data.address)");
          this.profileStorage.addresses.add(data.address);
        }
        break;
      }
      case "FormAutofill:RemoveAddresses": {
        log.debug("D: " + "this.profileStorage.addresses.remove(guid)");
   ...
   */
  _uninit() {
    log.debug("E: " + "this.profileStorage._saveImmediately()");
  ...
  _getAddresses({searchString, info}, target) {
    ...
    if (info && info.fieldName) {
      log.debug("F: " + "this.profileStorage.addresses.getByFilter({searchString, info})");
      addresses = this.profileStorage.addresses.getByFilter({searchString, info});
    } else {
    log.debug("G: " + "this.profileStorage.addresses.getAll()");
  ...
  _updateSavedFieldNames() {
    ...
    log.debug("H: " + "this.profileStorage.addresses.getAll()");
    ...
    log.debug("I: " + "this.profileStorage.INTERNAL_FIELDS");
